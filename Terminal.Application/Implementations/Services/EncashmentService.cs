...
public class EncashmentService : IEncashmentService
{
        ...
        ITmsClient tmsClient)
    {
        ...
        _tablesToSend = _configurationService.GetTablesToSend();
    ...
    public async Task EncashmentAsync()
    {
        ...
        foreach (var tableToSend in _tablesToSend)
        {
            ...
            {
                case "EventDb":
                    await ExportFromEventDbAsync(tableToSend, dbEvent);
                    break;
                
                default:
                    await ExportFromDataContextAsync(tableToSend, dbMain);
                    break;
            }
        }
    }

    private async Task ExportFromEventDbAsync(TableToSendDto tableToSend, EventDbContext dbEvent)
    {
    ...
    private async Task ExportFromDataContextAsync(TableToSendDto tableToSend, DataContext context)
    {
        switch (tableToSend.Name)
        {
            case "selling":
        ...
        where T : class
    {
        ...
        while (hasMore)
        {
            ...
            var (compressedFilePath, originalSize, compressedSize) = await SaveAndCompressBatchAsync(batch, table, batchNumber);
            
            await SendToTmsAsync(compressedFilePath, table, batchNumber, batch.Count, originalSize, compressedSize);
    ...
    private async Task<(string CompressedFilePath, long OriginalSize, long CompressedSize)> 
        SaveAndCompressBatchAsync<T>(
        List<T> data, 
        TableToSendDto table, 
        int batchNumber)
    {
        ...
        try
        {
            ...
            await using (var writer = new StreamWriter(jsonStream, System.Text.Encoding.UTF8, 8192, leaveOpen: true))
            {
    ...
    private async Task SendToTmsAsync(
