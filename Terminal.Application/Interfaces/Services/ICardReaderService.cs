...
public interface ICardReaderService : IDisposable
{
    ...
    /// <summary>
    /// Проверить доступность считывателя
    /// </summary>
    Task<bool> IsReaderAvailableAsync(CancellationToken cancellationToken = default);
    
    /// <summary>
    /// Получить версию драйвера/сервиса
    /// </summary>
    Task<string?> GetReaderVersionAsync(CancellationToken cancellationToken = default);
